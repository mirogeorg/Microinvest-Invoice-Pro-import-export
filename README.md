...
- Поддържа експорт и импорт на `Partners` чрез sheet `Партньори` (импортът подменя данните: опитва изтриване на съществуващите, маркира останалите с `Visible = 0`, и добавя нови записи с нов `PartnerID`, `MainPartnerID = PartnerID`, `Visible = 1`).
- Има функция за конвертиране на `Warehouse Pro` експорт на партньори (sheet `Partners`) към формат `Invoice Pro` (sheet `Партньори`) с мапинг:
  `Company -> Име`, `MOL -> Лице за контакт`, `TaxNo -> ДДС Номер`, `Bulstat -> Булстат`, `MainPartnerID = PartnerID`.
